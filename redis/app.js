(() => {
  ...
  const slides = window.REDIS_COURSE || [];
  ...
  function slideMarkup(item, i) {
    const visual = window.renderRedisVisual(item);
    return `<article class="slide mode-${esc(item.mode)}" data-stage="${esc(item.stage)}" data-index="${i}" aria-hidden="true">
      ...
      <div class="visual-board" data-mode="${esc(item.mode)}">${visual}
  ...
  function openDeep(component = null) {
    ...
    document.querySelector("#deep-related").innerHTML = related.map(row => `<button type="button" data-related-index="${row.i}"><span>${two(row.i + 1)}</span>${esc(row.slide.question)}</button>`).join("");
    const source = document.querySelector("#deep-source"); source.href = item.source;
  ...
  function openOverview() {
    lastFocus = document.activeElement; closeDeep();
    overview.classList.add("is-open"); overview.setAttribute("aria-hidden","false"); overview.inert = false;
  ...
  document.querySelector("#deep-toggle").addEventListener("click", () => deep.classList.contains("is-open") ? closeDeep() : openDeep());
  document.querySelector("#close-deep").addEventListener("click", closeDeep);
  scrim.addEventListener("click", closeDeep);
  ...
  window.addEventListener("keydown", event => {
    ...
    if (event.key === "Escape") { closeDeep(); closeOverview(); return; }
